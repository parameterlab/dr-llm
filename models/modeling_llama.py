...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        def continuous_router_decision(router_logits, p: float, deterministic=True):
            batch = router_logits.size(0)
            device = router_logits.device
            router_probs = F.softmax(router_logits, dim=-1)
            skip_probs = torch.tensor([1.0, 0.0, 0.0], device=device).expand_as(router_probs)
            exec_probs = torch.tensor([0.0, 1.0, 0.0], device=device).expand_as(router_probs)
            repeat_probs = torch.tensor([0.0, 0.0, 1.0], device=device).expand_as(router_probs)
            if p <= -0.5:
                # [-1, -0.5]: interpolate between skip and router
                t = (p + 1.0) / 0.5  # maps [-1,-0.5] -> [0,1]
                final_probs = (1 - t) * skip_probs + t * router_probs

            elif p <= 0.5:
                # (-0.5, 0.5]: interpolate between router and exec
                t = (p + 0.5) / 1.0  # maps [-0.5,0.5] -> [0,1]
                final_probs = (1 - t) * router_probs + t * exec_probs

            else:
                # (0.5,1]: interpolate between exec and repeat
                t = (p - 0.5) / 0.5  # maps [0.5,1] -> [0,1]
                final_probs = (1 - t) * exec_probs + t * repeat_probs

            final_probs = final_probs / final_probs.sum(dim=-1, keepdim=True)
            if deterministic:
                decisions = torch.argmax(final_probs, dim=-1)
            else:
                decisions = torch.multinomial(final_probs.squeeze(0), num_samples=1)
            return decisions, router_probs.squeeze(0)
        all_decision_logits = []
        all_decisions = []
        all_probs = []
        decode_layers = [
            self.layers[layer_idx] for layer_idx in self.layer_indices if layer_idx < self.config.num_hidden_layers
        ]
        input_indices = [hidden_states.shape[1] for _ in hidden_states]
        for i, decoder_layer in enumerate(decode_layers):
            ...
            decisions, probs = continuous_router_decision(decision_logits, p=self.router_penalty)
            # decisions = torch.argmax(decision_logits, dim=-1)
            decision = decisions.squeeze(-1).tolist()[0]
            if self.static_routing: decision = 1
            all_decisions.append(decision)
            all_probs.append(probs.squeeze(0).cpu().tolist())
